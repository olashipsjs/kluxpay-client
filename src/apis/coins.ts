...
export const getCoinList: QueryFunction = async ({ queryKey }) => {
  const [currency, category] = queryKey;

  try {
    const response = await fetch(
      `https://api.coingecko.com/api/v3/coins/markets?vs_currency=${currency}&category=${category}&per_page=12`,
      options
    );
...
export const getCoinData: QueryFunction = async ({ queryKey }) => {
...
export const getCoinChart: QueryFunction = async ({ queryKey }) => {
